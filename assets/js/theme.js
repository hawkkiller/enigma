    ...
    value: function initHighlight() {
      ...
      this.util.forEach(document.querySelectorAll('.highlight > .chroma'), function ($chroma) {
        ...
        if ($codeElements.length) {
          ...
          $title.classList.add('code-title'); // $title.insertAdjacentHTML('afterbegin', '<i class="arrow fas fa-chevron-right fa-fw" aria-hidden="true"></i>');
          // $title.addEventListener('click', () => {
          //     $chroma.classList.toggle('open');
          // }, false);

          $header.appendChild($title); // const $ellipses = document.createElement('span');
          ...
          if (_this5.config.code.copyTitle) {
            ...
            clipboard.on('success', function (_e) {
              _this5.util.animateCSS($code, 'animate__fadeIn');
            });
